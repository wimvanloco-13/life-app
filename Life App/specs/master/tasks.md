...
| 2026-05-15 | Habits UI — empty state redesign | Removed redundant "Habits" h1 and top-right "+ Add habit" button from the empty state. New `HabitEmptyState` owns the full page: centered title, subtitle, primary "Walk me through it" CTA, secondary "Add a habit" text link, and editorial principles below. PR #22. |
| 2026-05-15 | Habits UI — calendar strip + visible edit + principles sidebar | Replaced the 14-day single-row dot strip with a 2×7 calendar grid (HabitStrip): 32px cells showing day abbreviation and date number, week labels, today ring. Edit action moved from hidden kebab to always-visible pencil icon on HabitRow. HabitPrinciples extracted as shared component with `compact` prop; rendered in a 260px right sidebar via a `grid-cols-[1fr_260px]` layout in HabitList. PRs #23. |
| 2026-05-16 | Habits UI — 3-week HabitCalendar + card grid | New `HabitCalendar` component replaces `HabitStrip`: shows last / this / next calendar week (Mon–Sun), with week labels (Last/This/Next), day labels (Mo–Su), 44px cells, today ring, future dates muted/non-interactive. HabitRow redesigned as bordered card (`bg-card`, rounded-xl). HabitList uses `grid-cols-2 gap-4` so habits sit side-by-side. HabitPrinciples gains `horizontal` prop (grid-cols-3 below the grid, full width). PRs #24. |
| 2026-05-16 | Habits UI — full-width rows, identity left, calendar right | Dropped the 2-column card grid. Each habit is now a full-width row (no card border) with identity block on the left (w-52, Fraunces 15px) and the 3-week calendar filling the remaining width. Rows separated by thin dividers. Principles render with `mt-12` below the list. Skeleton updated to match. PR #25. |
| 2026-05-16 | Habits UI — page padding and typography polish | Added `px-6 py-8` outer padding to HabitList (matching Goals page). Identity text bumped to 17px; name subtitle to text-sm (14px); streak, day labels, week labels all increased. Left column widened to w-60; row gap to gap-12; row padding to py-8. Icons from w-3.5 to w-4. PR #26. |
| 2026-05-16 | Goal archive cascade + user reset script | Bug fix: `PATCH /api/goals/:id` now cascades `status` to monthly children when value is `"archived"` or `"active"` (restore). Completing a yearly goal does NOT cascade. New `scripts/reset-user-data.js`: Node.js utility (uses better-sqlite3) that deletes all data for a given username in FK-safe order while keeping the user account intact. PR #27. |
